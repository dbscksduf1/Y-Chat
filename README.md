...
- 메시지 조회 및 읽음 처리 과정에서 반복적인 DB 조회로 인한 N+1 문제가 발생
- JOIN 기반 DTO Projection을 적용하여 사용자 정보를 포함한 데이터를 단일 쿼리로 조회
...
- **문제**: 메시지 조회 및 읽음 처리 과정에서 반복적인 DB 조회로 인해 N+1 문제가 발생
- **원인**: 메시지 조회 후 사용자 정보 및 읽음 여부를 개별적으로 조회하는 구조
- **해결**: JOIN 기반 DTO Projection을 적용하여 메시지와 사용자 정보를 단일 쿼리로 조회하도록 개선하고, 반복 조회 구조를 제거  
- **결과**: 불필요한 쿼리 호출 횟수 감소
 
트래픽 증가 상황에 대비하여 효율적인 데이터 조회 구조의 중요성을 느꼈다.
